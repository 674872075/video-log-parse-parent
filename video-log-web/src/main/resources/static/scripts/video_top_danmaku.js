...
$(function () {
    $('#top_danmaku_li').attr('class', 'nav_active');
    init();
})
...
function init() {
        ...
        tooltip: {
            ...
            formatter: function (params) {
                ...
                for (let i = 0; i < params.length; i++) {
                    ...
                    if (params[i].seriesName == '弹幕') {
                        let danmaku = params[i].value;
                        if (danmaku >= 10000) {
                            danmaku = Math.round(danmaku / 1000) / 10;
                            danmaku = danmaku + '万';
                        }
                        list.push(
                            params[i].name +
    ...
    $.ajax({
        ...
        dataType: "json",
        //请求地址
        url: "/dwVideoTopDanmaku/queryAll",
    ...
    $.ajax({
        ...
        dataType: "json",
        //请求地址
        url: "/dwVideoTopDanmaku/getVideoCategoryCount",
        //data : JSON.stringify(list),
        success: function (result) {
            let rows = result.rows;
