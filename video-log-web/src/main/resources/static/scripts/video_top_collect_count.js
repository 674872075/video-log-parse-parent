...
$(function () {
    $('#top_collect_li').attr('class', 'nav_active');
    init();
})
...
function init() {
        ...
        tooltip: {
            ...
            formatter: function (params) {
                ...
                for (var i = 0; i < params.length; i++) {
                    ...
                    if (params[i].seriesName == '收藏量') {
                        let videoCollectCount = params[i].value;
                        if (videoCollectCount >= 10000) {
                            videoCollectCount = Math.round(videoCollectCount / 1000) / 10;
                            videoCollectCount = videoCollectCount + '万';
                        }
                        list.push(
                            params[i].name +
                            '<br/>' +
    ...
    $.ajax({
        ...
        dataType: "json",
        //请求地址
        url: "/dwVideoTopCollectCount/queryAll",
    ...
    $.ajax({
        ...
        dataType: "json",
        //请求地址
        url: "/dwVideoTopCollectCount/getVideoCategoryCount",
        //data : JSON.stringify(list),
        success: function (result) {
            let rows = result.rows;
